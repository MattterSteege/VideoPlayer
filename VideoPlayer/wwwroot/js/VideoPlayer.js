...
class VideoPlayer {
    ...
    //EVENTS
    callbacks = ["video_loaded", "audio_loaded", "loaded_manifest"];
    
    on(event, callback) {
        if (this.callbacks.includes(event)) {
            this.registeredCallbacks.push({event: event, callback: callback});
        }
        else {
            VideoError("Event not recognized: " + event);
        }
    }
    
    callRegisteredCallback(event, data) {
        if (!this.registeredCallbacks || this.registeredCallbacks.length === 0) return;
